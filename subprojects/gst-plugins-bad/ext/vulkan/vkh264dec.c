    ...
    GstQuery * query)
{
  ...
  GstVulkanVideoCapabilites vk_caps;
    ...
    GstVideoCodecFrame * frame, GstH264Picture * picture)
{
  ...
  pic = gst_vulkan_h264_picture_new (self, frame->output_buffer, NULL);
    ...
    GstH264Picture * first_field, GstH264Picture * second_field)
{
  ...
  second_pic = gst_vulkan_h264_picture_new (self, first_pic->base.out, NULL);
    ...
    const GstH264PPS * pps)
{
  ...
  if (!gst_vulkan_decoder_update_codec_parameters (self->decoder,
    ...
    GstVulkanDecoderPicture ** ref)
{
  ...
  *slot = (VkVideoReferenceSlotInfoKHR) {
  ...
  GST_INFO_OBJECT (self, "0x%lx slotIndex: %d", res->imageViewBinding,
      slot->slotIndex);
}

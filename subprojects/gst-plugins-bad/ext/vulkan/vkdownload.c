...
_image_to_raw_perform (gpointer impl, GstBuffer * inbuf, GstBuffer ** outbuf)
{
  ...
  gst_vulkan_operation_add_dependecy_frame (raw->exec, inbuf,
      VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);

  barriers = gst_vulkan_operation_create_barriers (raw->exec);
  cmd_buf = raw->exec->cmd_buf->cmd;

  gst_vulkan_operation_add_frame_barrier (raw->exec, inbuf, barriers,
  ...
  for (i = 0; i < n_planes; i++) {
    ...
    vkCmdCopyImageToBuffer (cmd_buf, img_mem->image, dst_layout,

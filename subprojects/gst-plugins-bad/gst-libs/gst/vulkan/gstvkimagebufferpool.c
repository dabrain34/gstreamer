...
prepare_buffer (GstVulkanImageBufferPool * vk_pool, GstBuffer * buffer)
{
  ...
  gst_vulkan_operation_add_dependecy_frame (priv->exec, buffer,
  ...
  gst_vulkan_operation_add_frame_barrier (priv->exec, buffer, barriers,
  ...
  if (barriers->len > 0) {
    if (gst_vulkan_operation_use_sync2 (priv->exec)) {
      ...
      priv->exec->vk_cmd_pipeline_barrier2 (priv->exec->cmd_buf->cmd,
          &dependency_info);
    } else {
  ...
  if (!gst_vulkan_operation_submit (priv->exec, &error))
    goto error;

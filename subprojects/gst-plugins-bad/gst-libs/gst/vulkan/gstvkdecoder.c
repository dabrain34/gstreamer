 ...
 * @short_description: Generic Vulkan Video Decoder
 */
...
static GstVulkanHandle *gst_vulkan_decoder_new_codec_parameters
    ...
    GstVulkanVideoProfile * profile, GError ** error)
{
  ...
  priv->empty_params =
      gst_vulkan_decoder_new_codec_parameters (self, &empty_params, error);
...
gst_vulkan_decoder_stop (GstVulkanDecoder * self)
{
  ...
  gst_clear_vulkan_handle (&priv->empty_params);
  gst_clear_vulkan_handle (&priv->codec_params);
    ...
    GstVulkanDecoderPicture * pic, GError ** error)
{
  ...
  if (!(priv->started && priv->codec_params))
    return FALSE;
  ...
  decode_start = (VkVideoBeginCodingInfoKHR) {
    .sType = VK_STRUCTURE_TYPE_VIDEO_BEGIN_CODING_INFO_KHR,
    .videoSession = priv->session.session->handle,
    .videoSessionParameters = priv->codec_params->handle,
  ...
  *cur_slot = pic->slot;
  ...
  if (!gst_vulkan_operation_add_dependecy_frame (priv->exec, pic->out,
  ...
  if (pic->dpb) {
    if (!gst_vulkan_operation_add_dependecy_frame (priv->exec, pic->dpb,
            VK_PIPELINE_STAGE_2_VIDEO_DECODE_BIT_KHR,
            VK_PIPELINE_STAGE_2_VIDEO_DECODE_BIT_KHR)) {
      return FALSE;
    }
  ...
  if (!self->layered_dpb) {
    ...
    for (i = 0; i < pic->decode_info.referenceSlotCount; i++) {
      ...
      if (!gst_vulkan_operation_add_dependecy_frame (priv->exec, ref_buf,
              VK_PIPELINE_STAGE_2_VIDEO_DECODE_BIT_KHR,
              VK_PIPELINE_STAGE_2_VIDEO_DECODE_BIT_KHR)) {
        return FALSE;
      }
    ...
    if (!gst_vulkan_operation_add_dependecy_frame (priv->exec,
  ...
  /* change image layout */
  barriers = gst_vulkan_operation_retrieve_barriers (priv->exec);
  /* *INDENT-OFF* */
  ...
  if (gst_vulkan_operation_get_query (priv->exec, (gpointer *) & query, error))
    GST_INFO_OBJECT (self, "query result: %d", query[0]);
...
gboolean
gst_vulkan_decoder_update_codec_parameters (GstVulkanDecoder * self,
    GstVulkanDecoderParameters * params, GError ** error)
{
  GstVulkanDecoderPrivate *priv;
  ...
  handle = gst_vulkan_decoder_new_codec_parameters (self, params, error);
  if (!handle)
    return FALSE;
    ...
    GstVulkanDecoderPicture * pic, GstBuffer * out)
{
  ...
  pic->img_view_out = gst_vulkan_decoder_picture_create_view (self, pic->out);
  ...
  if (self->layered_dpb) {
    pic->img_view_ref = gst_vulkan_decoder_picture_create_view (self,
        self->layered_buffer);
  } else if (self->dedicated_dpb) {
    ret = gst_buffer_pool_acquire_buffer (priv->dpb_pool, &pic->dpb, NULL);
    if (ret != GST_FLOW_OK)
      return FALSE;
    pic->img_view_ref = gst_vulkan_decoder_picture_create_view (self, pic->dpb);
  } else {

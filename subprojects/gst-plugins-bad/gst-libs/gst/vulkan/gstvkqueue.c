...
gst_vulkan_queue_create_decoder (GstVulkanQueue * queue, guint codec)
{
  ...
  if (device->properties.driverVersion < VK_MAKE_VERSION (1, 3, 238)) {
    GST_WARNING_OBJECT (queue,
        "Driver version doesn't support Video extensions");
    return NULL;
  }
 ...
 * @queue: a #GstVulkanQueue
 * @codec: the #GstVulkanVideoCodec to encode
 *
...
GstVulkanEncoder *
gst_vulkan_queue_create_encoder (GstVulkanQueue * queue,
    VkVideoCodecOperationFlagBitsKHR codec)
{
  ...
  if (device->properties.apiVersion < VK_MAKE_VERSION (1, 3, 254)) {
    GST_WARNING_OBJECT (queue,

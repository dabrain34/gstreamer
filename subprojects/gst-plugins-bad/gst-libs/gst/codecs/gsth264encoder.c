...
static void
_create_gop_frame_types (GstH264Encoder * self)
{
  guint i;
  ...
  g_assert (self->gop.idr_period <= MAX_H264_GOP_SIZE);
...
const gchar *
gst_h264_encoder_slice_type_name (GstH264SliceType type)
{
  switch (type) {
...
static void
_print_gop_structure (GstH264Encoder * self)
{
  ...
  for (i = 0; i < self->gop.idr_period; i++) {
    ...
    g_string_append_printf (str, "%s",
        gst_h264_encoder_slice_type_name (self->gop.frame_types[i].slice_type));

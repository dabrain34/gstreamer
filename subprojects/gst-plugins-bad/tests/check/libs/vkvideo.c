...
GST_START_TEST (test_decoder)
{
  ...
  /* *INDENT-ON* */
  GstVulkanVideoCapabilites video_caps;
  ...
  fail_unless (gst_vulkan_decoder_update_codec_parameters (dec,
          &create_params, &err));
  ...
  /* get input buffer */
  {
    ...
    slices_size = sizeof (slice);
    fail_unless (gst_vulkan_decoder_append_slice (dec, &pic, slice, slices_size,
            TRUE));
  }
  ...
  /* download image */
  {
    ...
    fail_unless (gst_vulkan_operation_start (exec, &error));
    gst_vulkan_operation_add_dependecy_frame (exec, pic.out,
        VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);

    barriers = gst_vulkan_operation_create_barriers (exec);
    gst_vulkan_operation_add_frame_barrier (exec, pic.out, barriers,
    ...
    fail_unless (gst_vulkan_operation_submit (exec, &error));
